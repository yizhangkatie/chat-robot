    ...
    return(
        <main className="w-full h-screen flex items-center justify-center homepage-bg">
            <div className="w-1/2 h-3/5 bg-white opacity-90 rounded-lg flex items-center justify-center">
                <FirstPage/>
            <form onSubmit={handleSubmit} className="w-1/2 h-full p-6 flex flex-col items-center">
                <p className="text-2xl pb-8 pt-12">{isLogin? '登录' :'注册'}</p>
                <div className="w-4/5 flex flex-col justify-start pb-6 pl-3">
                <label htmlFor="username" className="text-base pb-2">{isLogin? '用户名':'设置用户名'}：</label>
                <input
                    type="text"
                    id="username"
                    value={username}
                ...
                <label htmlFor="password" className="text-base pb-2 pt-4">{isLogin? '密码':'设置密码'}：</label>
                <input
                    type="password" 
                    id="password"
                    value={password}
                ...
                className="w-24 h-8 rounded-lg bg-blue-300">{isLogin ? '登录' : '注册'}</button>
                <button 
                type="button" 
